...
class StressClient(fl.client.NumPyClient):
    def __init__(self, cid: str, csv_path: str):
        self.cid = cid
        print(f"[Client {cid}] Using data file: {csv_path}")
        ...
        print(f"[Client {cid}] Detected input_dim = {input_dim}")
    ...
    def fit(self, parameters, config):
        ...
        self.model.train()
        local_epochs = int(config.get("local_epochs", 3))  # default 3
        ...
        for epoch in range(local_epochs):
            ...
            print(f"[Client {self.cid}] Epoch {epoch+1}/{local_epochs} - Local MSE: {avg_loss:.4f}")

        # Return updated parameters and size of training set
        return get_parameters_from_model(self.model), n_samples, {}

    def evaluate(self, parameters, config):
        # Optional: local eval; here we just skip or return 0
        set_parameters_to_model(self.model, parameters)
        self.model.eval()
        ...
        print(f"[Client {self.cid}] Eval MSE: {mse:.4f}")
...
def main():
    ...
    client = StressClient(cid=cid, csv_path=csv_path)

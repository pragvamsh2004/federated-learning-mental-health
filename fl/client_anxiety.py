...
class AnxietyClient(fl.client.NumPyClient):
    def __init__(self, cid: str, csv_path: str):
        self.cid = cid
        print(f"[Anxiety-Client {cid}] Using data file: {csv_path}")

        self.train_loader, input_dim = load_dass_csv(
            csv_path=csv_path,
            label_column="Anxiety_Score",
            batch_size=32,
            shuffle=True,
        )
        print(f"[Anxiety-Client {cid}] Detected input_dim = {input_dim}")
    ...
    def fit(self, parameters, config):
        ...
        for epoch in range(local_epochs):
            ...
            print(f"[Anxiety-Client {self.cid}] Epoch {epoch+1}/{local_epochs} - Local MSE: {avg_loss:.4f}")
    ...
    def evaluate(self, parameters, config):
        ...
        print(f"[Anxiety-Client {self.cid}] Eval MSE: {mse:.4f}")
...
def main():
    ...
    client = AnxietyClient(cid=cid, csv_path=csv_path)

# fl/server_depression.py  (DEPRESSION)
...
def get_evaluate_fn(test_csv_path: str):
    """Centralized eval on full DASS.csv using Depression_Score, and save final model."""
    ...
    def evaluate(server_round: int, parameters: List[np.ndarray], config):
        ...
        print(f"[Server-Depression] Round {server_round} - Eval MSE: {mse:.4f}")

        # 👉 On the final round, save the global model
        if server_round == NUM_ROUNDS:
            save_dir = os.path.join(ROOT_DIR, "saved_models")
            os.makedirs(save_dir, exist_ok=True)
            save_path = os.path.join(save_dir, "global_depression_model.pt")
            torch.save(model.state_dict(), save_path)
            print(f"[Server-Depression] Saved global model to: {save_path}")
